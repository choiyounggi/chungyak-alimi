...
def test_add_and_query(seeded):
    add_bookmark("applyhome:B1", session=seeded)
    assert bookmarked_pblanc_nos(session=seeded) == {"applyhome:B1"}
    assert is_bookmarked("applyhome:B1", session=seeded) is True
    assert is_bookmarked("applyhome:B2", session=seeded) is False
...
def test_idempotent_and_empty(seeded):
    ...
    add_bookmark("applyhome:B1", session=seeded)
    add_bookmark("applyhome:B1", session=seeded)  # 중복 추가 → 에러 없이 1건 유지
    assert bookmarked_pblanc_nos(session=seeded) == {"applyhome:B1"}
    remove_bookmark("applyhome:B1", session=seeded)
    remove_bookmark("applyhome:B1", session=seeded)  # 없는 것 삭제 → 에러 없음(멱등)
...
def test_bookmarked_dashboard_includes_unmatched(seeded):
    add_bookmark("applyhome:B2", session=seeded)  # B2는 매칭 안 됐지만 북마크됨
    items = bookmarked_dashboard(seeded)
    nos = [it["notice"].pblanc_no for it in items]
    assert nos == ["applyhome:B2"]  # 북마크된 것만
...
def test_bookmarked_dashboard_recency_order(seeded):
    ...
    seeded.add(Bookmark(pblanc_no="applyhome:B1", created_at=now - timedelta(minutes=5)))
    seeded.add(Bookmark(pblanc_no="applyhome:B2", created_at=now))
    seeded.commit()
...
def test_bookmark_put_delete_endpoints(seeded):
    c = TestClient(app)
    assert c.put("/bookmark/applyhome:B1").json() == {"bookmarked": True}
    with SessionLocal() as s2:
        assert is_bookmarked("applyhome:B1", session=s2) is True
    # 멱등: 다시 PUT 해도 200
    assert c.put("/bookmark/applyhome:B1").status_code == 200
    assert c.delete("/bookmark/applyhome:B1").json() == {"bookmarked": False}
    with SessionLocal() as s2:
        assert is_bookmarked("applyhome:B1", session=s2) is False
